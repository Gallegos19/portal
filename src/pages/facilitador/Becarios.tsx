...
import BecariosCreateDialog from '../admin/becarios/BecariosCreateDialog';
import BecariosDeleteDialog from '../admin/becarios/BecariosDeleteDialog';
import BecariosDetailPanel from '../admin/becarios/BecariosDetailPanel';
import BecariosEditDialog from '../admin/becarios/BecariosEditDialog';
import BecariosFilters from '../admin/becarios/BecariosFilters';
import BecariosTable from '../admin/becarios/BecariosTable';
import type { CreateInternForm, CreateUserForm } from '../admin/becarios/BecariosCreateDialog';
...
const Becarios: React.FC = () => {
  ...
  const loadData = React.useCallback(async () => {
    ...
    try {
      ...
      const [internResponse, userResponse, regionResponse, subprojectResponse] = await Promise.all([
      ...
      console.error('Error cargando becarios de facilitador:', error);
      setErrorMessage('No se pudieron cargar los becarios del facilitador.');
    } finally {
      setIsLoading(false);
    }
  }, [user?.id]);
  ...
  const filteredInterns = interns.filter((intern) => {
    const userData = userMap.get(intern.id_user);
  ...
  const handleOpenEdit = (intern: Intern) => {
    setEditForm({
      ...
      id_social_facilitator: currentFacilitator?.id ?? intern.id_social_facilitator
    });
    setIsEditOpen(true);
  };
  ...
  const resetCreateFlow = () => {
    ...
    setCreateUserErrors({});
    setCreateInternForm({
      ...initialCreateInternForm,
      id_social_facilitator: currentFacilitator?.id ?? ''
    });
  ...
  const handleSubmitEdit = async () => {
    ...
    if (!currentFacilitator?.id) {
      showToast('No se encontro el facilitador actual', 'error');
      return;
    }
      ...
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        <Box>
          ...
          <Typography variant="body2" color="text.secondary">
            Consulta y administra los becarios asignados a tu facilitacion social
          </Typography>
        </Box>
